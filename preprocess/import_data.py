...
def load_sensor_data(path2file=path.abspath("../data/sensor.csv"), num_rows=100):
    ...
    # Turbomachines are never designed to have a lot of variations in the system
    ...
    # STEP 7: Generate time series
    df_fwd = pandas.DataFrame(data=data_matrix[:, 1:-1])
    df_bwd = pandas.DataFrame(data=data_matrix[:, 1:-1][::-1])
    df_ewma_fwd = df_fwd.ewm(span=10).mean()
    df_ewma_bwd = df_bwd.ewm(span=10).mean()

    c = (df_ewma_bwd.values[::-1] + df_ewma_fwd.values) / 2.0
    data_matrix[:, 1:-1] = c
